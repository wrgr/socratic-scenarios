...
echo "== 2/3 unlearn ($METHOD, $DTYPE) on $MODEL =="
python unlearn.py --model "$MODEL" --method "$METHOD" --dtype "$DTYPE" --out "$OUT" $TARGETS_ARG $CHAT_ARG "$@"

echo "== 3/3 audit removal =="
python audit.py --model "$MODEL" --adapter "$OUT" --dtype "$DTYPE" $CHAT_AUDIT
...
in docs/novelty-and-positioning.md §8). In one shell:
  python serve.py --model "$MODEL" --dtype "$DTYPE"            --port 8000     # BASE
  python serve.py --model "$MODEL" --dtype "$DTYPE" --adapter "$OUT" --port 8001   # UNLEARNED
