...
smoothly to a $\le\!0.03$ floor across a dense 21-point LoRA-scale grid in \emph{four} independent
model families---Qwen2.5-3B, Phi-3.5-mini, and Zephyr-7B at three seeds each, Llama-3.1-8B at
one---with each family's transition knee at a different point on the gradient.
...
$\alpha$ sweep replicates across four model families---then use a decision-based domain, where the flat
...
facts into \emph{four} independent model families---four distinct pretraining lineages---drives mean
necessity smoothly down a dense 21-point LoRA-$\alpha$ grid ($0$ to $1$ in steps of $0.05$) to a
$\le\!0.03$ floor in every family (Figure~\ref{fig:factqa_dose}; a table subset in
Table~\ref{tab:dose}). Each family's transition knee sits at a \emph{different} $\alpha$ (Zephyr-7B
$\approx\!0.30$, Llama-3.1-8B $\approx\!0.35$, Qwen2.5-3B $\approx\!0.50$, Phi-3.5-mini
$\approx\!0.65$), so four architectures give the same collapse on their own schedules---ruling out a
single-model or single-schedule artifact---and the graded floor near zero is what a near-zero reading
on the real instrument is calibrated against. Every seed is monotone away from the endpoints: the only
non-monotonicities are $\le\!0.04$ wiggles \emph{at} the near-zero floor, plus Llama's naive end
starting at $0.96$ rather than $1.00$ (the base model guesses a probe correctly closed-book) before
reaching $1.00$ by $\alpha{=}0.1$. An earlier independent run of Qwen and Phi on a coarse 5-point grid
reproduces at the shared points ($0.48$ vs.\ $0.47$ and $0.93$ vs.\ $0.93$ at $\alpha{=}0.5$)---the
same instrument reading twice. This is the known-groups calibration, in a domain we did not
...
\centering
\begin{tikzpicture}[font=\footnotesize,
  cell/.style={draw=figink, line width=0.5pt, text width=46mm, align=center, inner sep=3pt, minimum height=12mm},
  clears/.style={cell, fill=figblue!12, draw=figblue, line width=1pt},
  lbl/.style={font=\scriptsize\bfseries, text=figink}]
\node[cell]                 (tl) at (0,0) {\textbf{grounds} — necessity $667$\\{\scriptsize naive: no fact to use}};
\node[cell, right=2mm of tl](tr)          {\textbf{grounds} — necessity $667$\\{\scriptsize \emph{knows but doesn't do}: recall $0.78$, decision unmoved}};
\node[cell, below=2mm of tl](bl)          {\textbf{grounds} — necessity $669.5$\\{\scriptsize reasoning alone: nothing to recall}};
\node[clears, right=2mm of bl](br)        {\textbf{clears} — necessity $0.3$\\{\scriptsize taught knowledge \emph{elicited} into the decision}};
\node[lbl, above=1.5mm of tl.north] {naive weights};
\node[lbl, above=1.5mm of tr.north] {taught weights (recall $0.00\!\to\!0.78$)};
\node[lbl, rotate=90, anchor=south, left=1mm of tl.west] {single-shot decision};
\node[lbl, rotate=90, anchor=south, left=1mm of bl.west] {reason-then-decide (CoT)};
\end{tikzpicture}
...
taught model} clears (bottom-right, $0.3$); reasoning on the naive model does not (bottom-left), so the
turn is the taught knowledge being elicited, not reasoning alone---the action needs \emph{both} the
knowledge and the elicitation. Installing the decision directly by fine-tuning does not reproduce this:
it yields a blanket/confabulated policy (\texttt{decision-teach-arc.md}), not the conditional fact.}
\label{fig:knowdo}
\end{figure}
...
shaded bands the min--max across seeds (Qwen/Phi $n{=}3$; Zephyr $n{=}3$ with two seeds stopped before
their high-$\alpha$ tails, so the band thins right of $\alpha\!\approx\!0.5$; Llama $n{=}1$). Four
independent lineages collapse to a $\le\!0.03$ floor with knees at four different $\alpha$---the
known-groups calibration that makes a near-zero necessity reading trustworthy.}
...
\midrule
Zephyr-7B ($n{=}3$)      & $1.00$ & $0.72$ & $0.09$ & $0.00^{\ast}$ & $0.00^{\ast}$ \\
Llama-3.1-8B ($n{=}1$)   & $0.96$ & $0.87$ & $0.08$ & $0.00$ & $0.00$ \\
Qwen2.5-3B ($n{=}3$)     & $1.00$ & $1.00$ & $0.47$ & $0.04$ & $0.03^{\ast}$ \\
Phi-3.5-mini ($n{=}3$)   & $1.00$ & $1.00$ & $0.93$ & $0.13$ & $0.01$ \\
\bottomrule
\end{tabular}
\caption{Necessity dose-response by construction (the calibration): mean necessity over available seeds
at a subset of the 21-point LoRA-$\alpha$ grid (the full grid is plotted in
Figure~\ref{fig:factqa_dose}). All four families collapse to a $\le\!0.03$ floor, with knees at four
different $\alpha$ (rows ordered by knee). Seeds are monotone away from the endpoints ($\le\!0.04$
wiggles at the floor only); Llama's $0.96$ at $\alpha{=}0$ is the base model guessing a probe
closed-book. $^{\ast}$fewer seeds at these points ($n{=}1$--$2$): two Zephyr seeds were stopped before
their high-$\alpha$ tails and one Qwen seed's final transcript was truncated in transfer.
...
\textbf{Uneven seed coverage.} The calibration dose-response spans four model families on the 21-point
grid, but coverage is uneven: Qwen2.5-3B, Phi-3.5-mini, and Zephyr-7B have three seeds (two Zephyr
seeds were stopped before their high-$\alpha$ tails, thinning that band right of
$\alpha\!\approx\!0.5$), Llama-3.1-8B has one, and a queued fifth family (OLMo-2-7B) was deliberately
cut to bound GPU cost. The knowing\,$\neq$\,doing flat-hazard result is three-seed. What remains
...
a graded dose-response ($1.00$ to a $\le\!0.03$ floor across a 21-point gradient, in a simulator-free
QA domain, replicated across four model families with knees at four different points). A decision-based
domain adds a validity caution the field should carry:
...
(\texttt{Qwen2.5-3B-Instruct}, \texttt{Phi-3.5-mini-instruct}, \texttt{zephyr-7b-beta}, and
\texttt{Llama-3.1-8B-Instruct} for the reported calibration; \texttt{Qwen2.5-3B-Instruct} for the
decision domain), seeds as reported per family in Section~\ref{sec:limits} (the decision-domain
follow-ups are seed~$0$), and the LoRA and SFT hyperparameters reported inline in
Section~\ref{sec:necessity}. All scoring is replayed off-GPU from the saved per-$\alpha$ transcripts;
