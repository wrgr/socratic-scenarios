...
\begin{tikzpicture}[font=\footnotesize,
  cell/.style={draw=figmuted, line width=0.5pt, text width=44mm, align=center, inner sep=4pt, minimum height=12mm},
...
\node[sharp, right=3mm of tl] (tr)         {\textbf{MISLED} / can't execute\\{\scriptsize call changed but still fails: the rule is wrong, or fix the model}\\{\scriptsize (the blind spot---Section~\ref{sec:diagnosis})}};
\node[cell, below=3mm of tl] (bl)          {\textbf{REDUNDANT}\\{\scriptsize same call either way, and it works---already known (prune)}};
\node[cell, right=3mm of bl] (br)          {\textbf{IGNORED} (unusable)\\{\scriptsize same call either way, and it fails---present, not used}};
\node[lbl, above=1.5mm of tl.north] {succeeds \emph{with} the item};
\node[lbl, above=1.5mm of tr.north] {fails \emph{with} the item};
\node[lbl, rotate=90, anchor=south, align=center, left=1mm of tl.west] {decision\\\emph{changes}};
\node[lbl, rotate=90, anchor=south, align=center, left=1mm of bl.west] {decision\\\emph{unchanged}};
\end{tikzpicture}
...
\centering
\begin{tikzpicture}[font=\footnotesize,
  cell/.style={draw=figmuted, line width=0.5pt, text width=46mm, align=center, inner sep=4pt, minimum height=13mm},
...
\node[cell, right=3mm of tl](tr)          {\textbf{grounds}\quad necessity $667$\\{\scriptsize \emph{knows but doesn't do}: recall $0.78$, decision unmoved}};
\node[cell, below=3mm of tl](bl)          {\textbf{grounds}\quad necessity $669.5$\\{\scriptsize reasoning alone: nothing to recall}};
\node[clears, right=3mm of bl](br)        {\textbf{clears}\quad necessity $0.3$\\{\scriptsize taught knowledge \emph{elicited} into the decision}};
\node[lbl, above=1.5mm of tl.north] {naive weights};
\node[lbl, above=1.5mm of tr.north] {taught weights (recall $0.00\!\to\!0.78$)};
\node[lbl, rotate=90, anchor=south, align=center, left=1mm of tl.west] {single-shot\\decision};
\node[lbl, rotate=90, anchor=south, align=center, left=1mm of bl.west] {reason-then-\\decide (CoT)};
% The bridge: same taught weights, elicitation flips the outcome.
\draw[-{Stealth[length=2.5mm]}, figblue, line width=1pt]
  (tr.east) to[bend left=42]
  node[right=1mm, align=left, font=\scriptsize, text=figblue]
...
shaded bands the min--max across three seeds (one Zephyr seed lacks its final four floor points, so
that band is $n{=}2$ there). Five independent lineages collapse to a $\le\!0.05$ floor with knees at
...
\midrule
Zephyr-7B      & $1.00$ & $0.72$ & $0.09$ & $0.00$ & $0.00^{\ast}$ \\
...
consistently across seeds. $^{\ast}$$n{=}2$: one Zephyr seed lacks its final four floor points.
...
\textbf{Seed coverage.} The calibration dose-response is five families $\times$ three seeds $\times$
21 grid points from a single clean run; the only gap is one Zephyr seed's final four floor points
($n{=}2$ there). The knowing\,$\neq$\,doing flat-hazard result is three-seed. What remains
