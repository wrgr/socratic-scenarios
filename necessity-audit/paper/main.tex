...
\emph{elicited} by reasoning, and it cannot be reliably \emph{installed} by direct fine-tuning, which
yields a shortcut policy rather than the conditional fact. On this calibrated
...
facts into \emph{two} independent model families drives mean necessity smoothly and monotonically down
the LoRA-$\alpha$ gradient: $1.00\!\to\!1.00\!\to\!0.48\!\to\!0.04\!\to\!0.05$ for Qwen2.5-3B and
$1.00\!\to\!1.00\!\to\!0.93\!\to\!0.13\!\to\!0.01$ for Phi-3.5-mini (three seeds each, monotone in every
seed; Table~\ref{tab:dose}, plotted with seed bands in Figure~\ref{fig:factqa_dose}). That two architectures give the same collapse---differing only in where
...
(accuracy with $-$ without corpus) falls to near zero down the LoRA-$\alpha$ gradient in every teaching
model family; shaded bands are the min--max across seeds. Two families collapsing on the same schedule
(a third architecture is added in the extended run) is the known-groups calibration that makes a
...
LoRA-$\alpha$ gradient in \emph{two} model families---monotone in every seed, differing only in where the
...
a graded dose-response ($1.00\!\to\!0.03$, in a simulator-free QA domain, replicated across two model
families and three seeds). A decision-based domain adds a validity caution the field should carry:
...
identifiers (\texttt{Qwen2.5-3B-Instruct}, \texttt{Phi-3.5-mini-instruct},
\texttt{Llama-3.1-8B-Instruct}), fixed seeds $\{0,1,2\}$, and the LoRA and SFT hyperparameters reported
