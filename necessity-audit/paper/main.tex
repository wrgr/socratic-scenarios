...
smoothly toward zero down the LoRA-scale gradient in two independent model families and three seeds
each ($1.00\!\to\!0.48\!\to\!0.05$ for Qwen2.5-3B, $1.00\!\to\!0.93\!\to\!0.01$ for Phi-3.5-mini).
...
$\alpha$ sweep replicates across two model families---then use a decision-based domain, where the flat
...
facts into \emph{two} independent model families drives mean necessity smoothly down the LoRA-$\alpha$
gradient to a near-zero floor: $1.00\!\to\!1.00\!\to\!0.48\!\to\!0.04\!\to\!0.05$ for Qwen2.5-3B and
$1.00\!\to\!1.00\!\to\!0.93\!\to\!0.13\!\to\!0.01$ for Phi-3.5-mini (three seeds each; five of the six
seeds are monotone throughout, and the exception is a small uptick \emph{at} the floor---one Qwen seed
rises $0.03\!\to\!0.11$ at $\alpha{=}1$---not a mid-curve reversal; Table~\ref{tab:dose}, plotted with
seed bands in Figure~\ref{fig:factqa_dose}). That two architectures give the same collapse---differing only in where
the knee sits---rules out a single-model artifact, and the graded floor near zero is what a near-zero
reading on the real instrument is calibrated against. This is the known-groups calibration, in a domain we did not
...
\caption{The fact-QA necessity dose-response, plotted (companion to Table~\ref{tab:dose}). Necessity
(accuracy with $-$ without corpus) falls to near zero down the LoRA-$\alpha$ gradient in every model
family we teach; shaded bands are the min--max across seeds. Independent families collapsing on the
same schedule---differing only in where the knee sits---is the known-groups calibration that makes a
near-zero necessity reading trustworthy.}
...
\bottomrule
\end{tabular}
\caption{Necessity dose-response by construction (the calibration), mean over three seeds. Teaching many
independent invented facts into the weights drives the instrument's necessity for them smoothly down the
LoRA-$\alpha$ gradient to a $\le\!0.05$ floor in \emph{two} model families---five of six seeds monotone
throughout (one Qwen seed ticks up $0.03\!\to\!0.11$ at the floor), differing only in where the
knee falls (the Qwen $\alpha{=}0.5$ point spans $0.39$--$0.56$ across seeds). That two architectures agree
rules out a single-model artifact. The answer-accuracy objective uses no simulator. The decision-based hazard
...
\textbf{Uneven seed coverage.} The calibration dose-response now spans two model families
(Qwen2.5-3B and Phi-3.5-mini) at three seeds each, and the knowing\,$\neq$\,doing flat-hazard result is
likewise three-seed---these are no longer single-run. What remains single-seed (seed~$0$) is the pair of
decision-domain \emph{follow-ups}: the CoT-elicitation bridge ($\approx\!0.3$ elicited) and the
direct-install (decision-format SFT) arc, whose point values we treat as single-run evidence, keeping the
corresponding hedging in Section~\ref{sec:necessity}. Provenance and the remaining queue are in
\texttt{experiments/provenance/} (\texttt{decision-teach-arc.md}, \texttt{gpu-pending.md}); the offline
mock recovery already confirms the \emph{instrument} recovers ground truth on these domains.
...
a graded dose-response ($1.00$ to a $0.01$--$0.05$ floor, in a simulator-free QA domain, replicated
across two model families and three seeds). A decision-based domain adds a validity caution the field should carry:
...
calibration and the decision-domain follow-ups) are driven by one orchestrator over pinned model
identifiers (\texttt{Qwen2.5-3B-Instruct} and \texttt{Phi-3.5-mini-instruct} for the reported
calibration; \texttt{Qwen2.5-3B-Instruct} for the decision domain), seeds $\{0,1,2\}$ for the
calibration and hazard sweeps (the decision-domain follow-ups are seed~$0$;
Section~\ref{sec:limits}), and the LoRA and SFT hyperparameters reported
inline in Section~\ref{sec:necessity}; per-run logs are retained as provenance. Live-model results
