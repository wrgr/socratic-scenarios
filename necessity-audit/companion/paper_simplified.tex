...
it in \emph{four} different model families from four different labs (Qwen, Phi, Zephyr, and Llama),
and all four give the same clean ``dose-response'' curve---sliding from fully corpus-reliant to
almost self-sufficient, each with its drop-off at a \emph{different} point on the dial. Four unrelated
models agreeing, each on its own schedule, is strong evidence it isn't a quirk of one. It uses no
