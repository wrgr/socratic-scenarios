...
real models, a bit at a time, and necessity falls \textbf{smoothly and steadily toward zero} as the
model soaks the facts up---smooth \emph{because there are many independent facts} (learn a quarter,
then half, then almost all). We ran it in \emph{two} different model families (Qwen and Phi), three
seeds each, and both give the same clean ``dose-response'' curve---sliding from fully corpus-reliant
to almost self-sufficient, differing only in exactly where the drop-off sits. That two unrelated
models agree is the evidence it isn't a quirk of one. It uses no simulator, so the method isn't a
lucky trick of one hand-built environment; it works on any task with a checkable right answer.
