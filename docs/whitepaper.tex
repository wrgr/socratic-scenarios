...
\title{\vspace{-2em}\Huge\bfseries\sffamily\color{tmink} TeachMe AJP \\[0.4em]
  \Large\normalfont\color{tmmuted} Retrieval-Augmented Operational Training for the\\ Optomec HD2 Aerosol Jet Printer}
\date{\normalsize\sffamily\color{tmmuted}\today}
\author{}
...
\begin{titlepage}
  \centering
  \vspace*{3cm}
  {\Huge\bfseries\sffamily\color{tmink} TeachMe AJP\par}
  \vspace{0.6em}
  {\Large\sffamily\color{tmmuted} Retrieval-Augmented Operational Training for the\\ Optomec HD2 Aerosol Jet Printer\par}
  \vspace{2em}
  \begin{tcolorbox}[colback=tmaccent!8, colframe=tmaccent!45, width=0.55\textwidth, sharp corners, boxrule=0.6pt]
    \centering\small\sffamily\color{tmink} RESEARCH PROTOTYPE --- not for operational use without expert review
  \end{tcolorbox}
  \vspace{3cm}
  {\sffamily\color{tmmuted}\today\par}
  \vfill
  {\small\sffamily\color{tmmuted} A domain instantiation of the TeachMe / EDDIE corpus-bound Socratic training platform.\par}
\end{titlepage}
...
\begin{tikzpicture}[
  ...
  lbl/.style={font=\scriptsize\bfseries\sffamily, text=tmmuted},
  arr/.style={-Stealth, thick, tmink}
]
  ...
  \node[lbl] (graphlbl) at (-6.4, -6.3) {Graph Tier (always available)};
  \node[smallbox] (gm) at (-6.4, -7.6) {\code{matchNodesSemantic()} \\ cosine similarity over pre-embedded node content (Jaccard fallback)};
  \node[smallbox] (trav) at (-6.4, -9.7) {Edge traversal: Symptom \arrowto{} FailureMode \arrowto{} CorrectiveAction / SafetyHazard};

  \node[lbl] (denselbl) at (0, -6.3) {Dense Tier (requires \texttt{npm run ingest})};
  \node[smallbox] (embi) at (0, -7.6) {\code{embed(query)}};
  \node[smallbox] (cos) at (0, -9.7) {cosine similarity vs.\ precomputed 3072-D chunk vectors};

  \node[lbl] (routerlbl) at (6.4, -6.3) {Router Strategies (mode-specific)};
  \node[smallbox, text width=4.4cm, minimum height=1.9cm] (router) at (6.4, -7.9) {fault-diagnosis \textbullet\ step-context \textbullet\ probe-context \textbullet\ safety-gate \textbullet\ tacit-lookup};

  \node[box] (hybrid) at (-3.4, -12.1) {\textbf{HybridResult} \\ InOperationView};
  \node[box] (mentor) at (3.4, -12.1) {\textbf{Mentor LLM} \\ SocraticView, ScenarioView};
...
\begin{tikzpicture}[
  ...
  \node[box] (pe) at (5.4, 0) {\textbf{Prompt Enricher} \\ rewrites vague queries; HITL review before retrieval fires};

  \node[box] (mentor) at (2.7, -4.6) {\textbf{Mentor Agent} \\ asks \textbullet\ probes \textbullet\ scaffolds \textbullet\ evaluates \textbullet\ advances or re-teaches};
  \node[box] (narrator) at (8.1, -4.6) {\textbf{Narrator Agent} \\ plays the machine; reports outcomes; corpus only, never generates};

  \node[kgbox] (kg) at (5.4, -7.8) {\textbf{Knowledge Graph} \\ 100+ typed nodes: Equipment, Steps, Parameters, Faults, Symptoms, Remedies, Safety, Tacit, Probes};

  \draw[arr] (learner.east) -- node[above, font=\tiny] {types action/answer} (pe.west);
  \draw[arr] (pe.south) -- node[above, sloped, font=\tiny, fill=white, inner sep=1pt] {enriched query} (mentor.north east);
  \draw[biarr] (mentor.east) -- node[above, font=\tiny] {shares step context} (narrator.west);
  \draw[arr] (narrator.south) to[out=250, in=290, looseness=2.4] node[pos=0.5, below=1pt, font=\tiny, fill=white, inner sep=1pt] {reports observable machine behavior} (learner.south);
  \draw[arr] (mentor.south) -- node[left, font=\tiny] {ZPD-aware retrieval} (kg.north west);
  \draw[arr] (narrator.south) -- node[right, font=\tiny] {deterministic lookup} (kg.north east);
\end{tikzpicture}%
}
...
\begin{center}
\large\bfseries\sffamily\color{tmaccent} Signals \arrowto{} Retrieval \arrowto{} Mentor Loop \arrowto{} Safety Gate \arrowto{} Transfer
\end{center}
