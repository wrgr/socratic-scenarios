...
\addlinespace
\textbf{CS4} validations (does the instrument hold up?) & \S\ref{sec:instrument},\,\ref{sec:unlearning} &
...
function of the \emph{(question, retrieved context, answer)} triple, computed with the corpus
\emph{present}. Necessity is a \emph{contrast}---$\mathrm{acc}(\text{with corpus})-\mathrm{acc}(\text{without
corpus})$---and RAGAS never runs the without-corpus condition, so it is \emph{structurally blind}
to the quantity, regardless of how the answers are phrased. A model that already knows a fact and
one that does not \emph{both} answer correctly with the corpus present and both score \emph{high}
(the correct answer is entailed by the context whether or not the model used it); a metric that
only ever sees the with-corpus arm therefore cannot separate a load-bearing corpus from a
redundant one, and reports the redundant case as sufficient---the false-sufficiency failure our
measure is built to catch. (Our released harness computes both the RAGAS scores and our necessity
contrast side by side; the point is definitional---the without-corpus arm RAGAS omits is exactly
what necessity is read from.) \textbf{ContextCite} \citep{contextcite2024} is the closest relative: because
...
that supersedes the earlier single-hazard $\delta_{\regret}$ scan (retained for provenance as
Appendix~\ref{app:legacy}, Table~\ref{tab:disc-legacy}). Five production models run over AWS
...
reliance is a fraction (CS1.4's fraction-valued fix; the legacy single-hazard $\delta_{\regret}$ scan is
Table~\ref{tab:disc-legacy}). Classification uses \emph{necessity} $=\regret(\text{ablated})-\regret(\text{with-corpus})$
...
% The legacy single-hazard necessity--usability plane (Figure~\ref{fig:plane-legacy})
% now lives in Appendix~\ref{app:legacy}, together with the legacy eight-model table.
...
vs.\ RAG-as-reasoning-substrate) made measurable. \emph{As in CS2.1, these use hand-built reference
policies with known ground truth}, so the run validates the \emph{instrument}; a real model is the
\emph{subject}, run through the same rig (the live path is wired).

\paragraph{CS2.2 (live) --- a real model reads as a reasoner.} Because the learners above are hand-built
reference policies, that run validates the \emph{instrument}; putting a live model through the \emph{same}
...
what the ensemble's variance control flags as noise rather than reliance. The reference policies validate
the instrument; the live model is the subject measured on it. Provenance:
...
\emph{three} provider families (Anthropic, Meta, Amazon) on an eight-hazard suite; the legacy
single-hazard scan (Appendix~\ref{app:legacy}) additionally spans eight models and records why two open
reasoning models were excluded as unscoreable by the single-shot instrument---and reports the head-on rule; the
...
\section{Legacy single-hazard cross-model scan (superseded by the CS1.4 suite)}
\label{app:legacy}
An earlier version of the cross-model discrimination (CS1.3) used a \emph{single} corpus-only charted
hazard and reported necessity as a coarse geometric ordinal $\delta_{\regret}$ rather than as a fraction
over a suite of independent hazards. We retain it here for provenance; the fraction-valued suite of
Table~\ref{tab:disc} (CS1.4's fix) supersedes it in the main text. This legacy scan also covered a wider
model set (eight models, plus two open reasoning models run and excluded), so it is the reference for the
model-count and exclusion statements in the main text.

\emph{(1) Standard rules are redundant for these frontier models.} Every model relies on little of the
standard corpus (relied $0$--$1$ of $4$; it duplicates their parametric COLREG---\emph{redundant}, an
actionable ``do not ship'' verdict). We also ran two open reasoning models (OpenAI \texttt{gpt-oss}
20B/120B) that \emph{appeared} to rely on the same corpus (relied $2$--$3$/$4$), but we exclude them from
Table~\ref{tab:disc-legacy}: the single-shot instrument cannot score them reliably. Their harmony-format
reasoning frequently failed to terminate within the decode budget (the 20B needed a $50$k-token budget
where every other model used $<\!4$k), and with no repeat/variance control an unstable output yields
ablation deltas indistinguishable from genuine reliance. Whether such models truly rely on standard
COLREG---i.e.\ whether necessity is a property of the (corpus, learner) pair and not the corpus alone---is
a real question, but confirming it needs a variance-controlled, parse-budget-matched rerun (future work).
...
\emph{(3) The redundant-vs-unusable split shows on real models} (Figure~\ref{fig:plane-legacy}). With-corpus
regret separates \emph{usable} from \emph{unusable}: all clear once given the rule ($\regret_{\text{with}}\!\approx\!0.2$)
\emph{except} Llama-3.3-70B ($457$) and Nova-micro ($670$), which ground \emph{even with the rule
present}---the content has value but the model cannot act on it (\textsc{unusable}; fix the model, not
the corpus). This is genuine learner-dependence: the same corpus item is \emph{usable} for one model and
\emph{unusable} for another. (A prompt-condition study on Gemini gives the binding-fidelity control:
under a strict binding prompt necessity grades monotonically with capability; removing the clause flips
every model to \textsc{leaking}.) The continuous $\delta_{\regret}$ and $\regret_{\text{with}}$ are more
reliable than the categorical verdict, a noisier three-signal majority vote.
...
\bottomrule
\end{tabular}
\caption{\textbf{Legacy (superseded).} Cross-model necessity on AWS Bedrock (eight models),
single-hazard scan. \textbf{relied/4}: standard-COLREG rules the model relies on, of four (per-item
audit); $0$--$1$ reads \emph{redundant}/false-sufficiency. \textbf{Hazard}: $\delta_{\regret}$ is
necessity (regret rise when the corpus-only rule is ablated); $\regret_{\text{with}}$ is regret \emph{with}
the rule present ($\approx\!0.2$ = clears, so \emph{usable}; $\gg\!0$ = grounds anyway = \textsc{unusable}).
Standard COLREG is \emph{redundant} across all eight (relied $0$--$1$/4). On the hazard, $\delta_{\regret}$
ranges from $0$ (Nova-micro ignores it) to $1998$ (Opus, full-barrier \textsc{corpus-bound}); this number
is a coarse ordinal set by the fixed sim geometry (identical maneuvers therefore yield identical
$\delta_{\regret}$---four models emit the same $30^\circ$ avoidance and land at $669$), not a fine-grained
per-model scalar. $\regret_{\text{with}}$ exposes two models that ground even with the rule
(\textsc{unusable}). Mock learners bound the scale. (Two open \texttt{gpt-oss} reasoning models were
excluded: their reasoning did not reliably terminate within the decode budget, so the single-shot
instrument cannot score them without a variance control---see text.) Superseded by the fraction-valued
suite of Table~\ref{tab:disc}.}
\label{tab:disc-legacy}
\end{table}
...
\begin{axis}[
  ...
  xmin=-90, xmax=2260, ymin=-60, ymax=800,
  xtick={0,500,1000,1500,2000}, ytick={0,200,400,600},
...
\addplot[only marks, mark=*, mark size=1.6pt, figorange]
  coordinates {(0,670) (213,457)};
\node[font=\scriptsize, anchor=south] at (axis cs:1998,8) {Opus};
\node[font=\scriptsize, anchor=south] at (axis cs:660,8) {Haiku/Sonnet/Nova-pro/8B};
\node[font=\scriptsize, anchor=south] at (axis cs:145,8) {Nova-lite};
\node[font=\scriptsize, anchor=south] at (axis cs:0,685) {Nova-micro};
\node[font=\scriptsize, anchor=west]  at (axis cs:235,457) {Llama-70B};
\node[figblue, font=\scriptsize\itshape, anchor=north east] at (axis cs:2240,120) {corpus-bound};
\node[figorange, font=\scriptsize\itshape, anchor=north east] at (axis cs:2240,760) {unusable: grounds even with the rule};
\end{axis}
\end{tikzpicture}
\caption{\textbf{Legacy (superseded).} The necessity--usability plane for the eight models of
Table~\ref{tab:disc-legacy} on the single corpus-only hazard. \emph{Horizontal:} necessity
$\delta_{\regret}$ (how much ablating the rule raises regret; a coarse ordinal set by the fixed
geometry---the four blue points at $\delta_{\regret}\!\approx\!668$ are four models that emit the same
avoidance angle). \emph{Vertical:} with-corpus regret---$\approx\!0$ (blue, on the axis) means the model
\emph{clears} once given the rule (\emph{usable}); a large value (orange) means it \emph{grounds even with
the rule present} (\emph{unusable}). Reliance and usability are distinct axes: Claude Opus reads the
corpus-only fact and clears, while Nova-micro and Llama-70B ground regardless.}
\label{fig:plane-legacy}
\end{figure}
