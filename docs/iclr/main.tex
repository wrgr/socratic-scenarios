...
three scorable conditions: \textsc{none} (closed-book), \textsc{retrieved} (the deployed dense
retriever's top-$k$), and \textsc{oracle} (the answer-bearing chunk). Accuracy across them attributes the
gain: $\mathrm{acc}(\textsc{oracle}){-}\mathrm{acc}(\textsc{none})$ is the \emph{content ceiling},
$\mathrm{acc}(\textsc{retrieved}){-}\mathrm{acc}(\textsc{none})$ what the pipeline \emph{delivers}, and
their difference the \emph{retrieval gap}. On seven equipment-SOP fact-recall queries through a small
local generator (Qwen2.5-1.5B), closed-book is $0/7$, \textsc{oracle} $7/7$, and the deployed MiniLM
pipeline $3/7$---so of a $100\%$ content ceiling it delivers $43\%$ and forfeits a $57\%$ retrieval gap
(generator-independent recall@$5$ $4/7$ localizes it). The binding constraint is retrieval, not coverage
or capability---distinct outcomes the binary leakage verdict cannot separate. (One small model, seven
queries: the method, not a benchmark. Harness: \texttt{experiments/retrieval-decomp}.)
