...
\emph{(2) Read on necessity and regret-with, not the verdict.} We classify on \emph{necessity}
$=\regret(\text{ablated})-\regret(\text{with-corpus})$ and \emph{regret-with}, not the categorical leakage
verdict---a noisier majority vote that also demands following the inverted counterfactual and abstaining
closed-book, so it \emph{undercounts} a model that reads the fact but answers closed-book from priors: an
earlier verdict-based count wrongly put Haiku at $0$, where necessity credits it with $4$. The verdict is
retained only for the standard-COLREG strict-mode reading below.
