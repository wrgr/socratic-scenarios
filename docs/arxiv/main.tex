...
context-level one. The harness is built, and a first CPU result already delivers the
audit-level half: with the primary method SimNPO on Qwen2.5-1.5B-Instruct---a model that
knows the rule---unlearning drives the forget-target NLL from $4.6$ to $92$ and the held-out
...
($4.6\to41$, $5/6\to0/6$), so the suppression is method-agnostic. This establishes behavioral
\emph{suppression}, not yet verified semantic \emph{removal}: the probe is lexical and the
NLL teacher-forced. Scoring the base-vs.-unlearned model on the full regret instrument at
7--8B scale---the step that turns suppression into an instrument-level removal result and
fills the corpus-recovery cell of the $2\times2$---remains GPU-gated.
...
audit-level unlearning result is in hand (CPU), but the instrument-scoring step of the
open-weight arm (Section~\ref{sec:weightlevel})---which settles \emph{removal}---still needs one GPU.
