...
\node[draw=figorange, rounded corners=2pt, line width=0.8pt, dashed, fill=figorange!5,
  ...
  {\footnotesize\textbf{One metric, read three ways.}\;
   \textbf{C2 identifiability:} ablate a knowledge component $\Rightarrow$ exactly one metric moves.\;
   \textbf{C1 localize:} a task failure $\Rightarrow$ which rule is missing.\;
   \textbf{C1 leakage:} ablate a rule $\Rightarrow$ $J$ jumps $=$ corpus-bound, $J$ unchanged $=$ leaking.};
...
identifiability (C2), to localize a missing rule from task failures (C1), and to detect
parametric leakage by rule ablation (C1)---needing only API access to the learner.}
...
The sharpest objection to C1 is that corpus \emph{context} removal is not weight-level
\emph{unlearning}, so the ``leakage'' test could be the no-context heuristic in disguise. We
...
elliptical-domain \emph{form} itself, is untested. \textbf{Model and domain coverage.}
