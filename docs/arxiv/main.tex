...
charted hazard} of Section~\ref{sec:construction} (Table~\ref{tab:disc}). Three findings.
...
\emph{(3) The redundant-vs-unusable split shows on real models.} With-corpus regret separates
\emph{usable} from \emph{unusable}: all clear once given the rule ($\regret_{\text{with}}\!\approx\!0.2$)
...
\paragraph{Why the $2\times2$ is flat.} Rule~14 is already \textsc{leaking} at baseline---every
pretrained model knows head-on\,$\to$\,starboard (Section~\ref{sec:diagnosis})---so ablating it
