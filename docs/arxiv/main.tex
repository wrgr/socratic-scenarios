...
\textbf{Unlearning is a single model at one scale.} The instrument-scoring result
(Section~\ref{sec:weightlevel}) is informative---a gentle recipe suppresses the \emph{words}
while the scored \emph{decision} is untouched (says\,$\neq$\,does), and an aggressive one
\emph{inverts} the rule---but it is one 3B model; seed/method variance and replication at a
larger scale (7B, \texttt{bfloat16}, not 4-bit---quantization noise would confound the
damage/coherence signal that is the point) are outstanding. A clean
\textsc{corpus-bound}\,$\to$\,\textsc{gone} transition is structurally out of reach for any
pretrained COLREG rule (all leak); demonstrating it requires the corpus-only rule of the
designed next experiment, not a re-tune of this one.
