...
1.5 redundant-vs-unusable; 1.6 corpus-value audit/localization; 1.7 hazard dose-response
...
\paragraph{Real-model discrimination (CS1.3).} We run the necessity test on eight production models over AWS
Bedrock---Claude Haiku/Sonnet/Opus~4.5, Llama-3.1-8B / 3.3-70B, and Nova micro/lite/pro---contrasting a
\emph{standard-COLREG} corpus with the \emph{corpus-only charted hazard} of Section~\ref{sec:construction}
(Table~\ref{tab:disc}, Figure~\ref{fig:plane}). Three findings.
...
Table~\ref{tab:disc}: the single-shot instrument cannot score them reliably. Their harmony-format
...
\bottomrule
\end{tabular}
\caption{Cross-model necessity on AWS Bedrock (eight models). \textbf{relied/4}: standard-COLREG rules
the model relies on, of four (per-item audit); $0$--$1$ reads \emph{redundant}/false-sufficiency.
\textbf{Hazard}: $\delta_{\regret}$ is necessity (regret rise when the corpus-only rule is ablated);
$\regret_{\text{with}}$ is regret \emph{with} the rule present ($\approx\!0.2$ = clears, so \emph{usable};
$\gg\!0$ = grounds anyway = \textsc{unusable}). Standard COLREG is \emph{redundant} across all eight
(relied $0$--$1$/4). On the hazard, $\delta_{\regret}$ ranges from $0$ (Nova-micro ignores it) to $1998$
(Opus, full-barrier \textsc{corpus-bound}); this number is a coarse ordinal set by the fixed sim geometry
(identical maneuvers therefore yield identical $\delta_{\regret}$---four models emit the same $30^\circ$
avoidance and land at $669$), not a fine-grained per-model scalar. $\regret_{\text{with}}$ exposes two
models that ground even with the rule (\textsc{unusable}). Mock learners bound the scale. (Two open
\texttt{gpt-oss} reasoning models were excluded: their reasoning did not reliably terminate within the
decode budget, so the single-shot instrument cannot score them without a variance control---see text.)}
\label{tab:disc}
\end{table}
...
\end{axis}
\end{tikzpicture}
\caption{The necessity--usability plane for the eight models of Table~\ref{tab:disc} on the corpus-only
hazard. \emph{Horizontal:} necessity $\delta_{\regret}$ (how much ablating the rule raises regret; a
coarse ordinal set by the fixed geometry---the four blue points at $\delta_{\regret}\!\approx\!668$ are
four models that emit the same avoidance angle). \emph{Vertical:} with-corpus regret---$\approx\!0$
(blue, on the axis) means the model \emph{clears} once given the rule (\emph{usable}); a large value
(orange) means it \emph{grounds even with the rule present} (\emph{unusable}). Reliance and usability are
distinct axes: Claude Opus reads the corpus-only fact and clears, while Nova-micro and Llama-70B ground
regardless.}
\label{fig:plane}
\end{figure}
...
validity claim at scale. The discrimination result (Table~\ref{tab:disc}) covers eight models across
\emph{three} provider families (Anthropic, Meta, Amazon)---two open reasoning models were run but
excluded as unscoreable by the single-shot instrument (\S\ref{sec:diagnosis})---and reports the head-on rule; the
