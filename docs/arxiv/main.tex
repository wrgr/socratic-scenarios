...
\begin{axis}[
  ...
  xmin=-0.4, xmax=6.4,
  xtick={0,1,2,3,4,5,6},
  xticklabels={$\varnothing$,{+role},{+stbd},{+subst},{+early},{+safe-spd},{+multi}},
...
within \emph{one} provider family; the ablation and unlearning results turn on \emph{two}
rules (Rule~14 head-on, Rule~15 crossing) and a single unlearned model family. A persuasive
claim needs cross-provider models and many more rules/encounters---which is why we release the
harness (\texttt{experiment.sh}) built to run exactly that sweep, and report these as worked
