...
\emph{except} Llama-3.3-70B ($457$), Nova-micro ($670$), and \texttt{gpt-oss}-120B ($431$), which ground
\emph{even with the rule present}---the content has value but the model cannot act on it
(\textsc{unusable}; fix the model, not the corpus). (A prompt-condition study on Gemini gives the
...
\texttt{gpt-oss}-20B  & $\mathbf{3}$ & \multicolumn{2}{c}{\emph{n/a}} & \emph{n/a} \\
\bottomrule
\end{tabular}
...
exposes three models that ground even with the rule. Mock learners bound the scale; \texttt{gpt-oss}-20B's
hazard cell is \emph{n/a}---on the harder hazard scenario its analysis-channel reasoning did not
terminate within the decode budget, a capability-linked effect absent in the larger 120B. The
continuous $\delta_{\regret}$/$\regret_{\text{with}}$ are more reliable than the categorical verdict.}
\label{tab:disc}
\end{table}
