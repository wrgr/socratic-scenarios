...
gap}---value present in the corpus but never surfaced. A generator-independent retrieval read
(recall@$k$ of \emph{any} answer-bearing chunk) localizes that gap directly: on a terse
equipment-SOP corpus, a MiniLM retriever surfaces the needed fact for only $4/7$ held-out
fact-recall queries at $k{=}5$, so a substantial share of curated value is lost to
\emph{retrieval}, not to the model or to the corpus's contents. This separates a curation
failure (fact absent) from a retrieval failure (fact present but unranked) from a
model-priors success---distinctions the binary leakage verdict cannot make.
