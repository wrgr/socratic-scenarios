...
To demonstrate the instrument detecting an unlearning-\emph{induced} policy change, the target
rule must be \emph{corpus-bound at baseline} (high ablation-delta)---which no pretrained COLREG
rule is. The designed next experiment therefore introduces a \emph{corpus-only} rule with no
pretraining support: a fictional-jurisdiction stricter-speed rule on the already-instrumented
speed axis---e.g.\ ``in the Xylos Strait under restricted visibility, reduce to bare
steerage''---a novel numeric threshold a model cannot know from priors, and deliberately
\emph{not} a safety inversion. A model taught only this rule should read \textsc{corpus-bound}
(it follows the corpus) against a prior-driven model that ignores it; unlearning it should then
move the $2\times2$ cell \textsc{corpus-bound}\,$\to$\,\textsc{gone}. We present this as the
designed next experiment (implementation in \texttt{experiments/unlearning} plus the
instrument's speed probe), not a completed result.
