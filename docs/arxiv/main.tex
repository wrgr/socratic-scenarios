...
estimator-recovery calibration, and a real-model discrimination result: on two current
LLMs the instrument diagnoses corpus-binding under a strict prompt and, when the binding
clause is removed, detects the leakage signal (ablation-$\delta$ collapse and closed-book
contamination), with the composite verdict flipping fully to \emph{leaking} on the lighter
model. All results are \textbf{simulation-based mechanism evidence, not human-subject
...
steering task---avoiding ship collisions---where we can compute the mathematically best
move, and scores the tutor by how far its decisions fall from that best move. Because we
...
\paragraph{Real-model discrimination.} We run the instrument on two current production
LLMs under both prompt conditions---\texttt{bound} (the strict corpus-binding positive
control) and \texttt{unconstrained} (the binding clause removed, parametric fallback
allowed)---and read the verdict off the same metric (Table~\ref{tab:disc}). Under
\texttt{bound}, both models are diagnosed \textsc{corpus-bound}: a large ablation-delta,
counterfactual adherence, and closed-book abstention. Under \texttt{unconstrained}, the
central leakage signal appears for \emph{both} models---the ablation-delta collapses to
$0.000$ (the governed metric no longer depends on the corpus) and the closed-book baseline
flips from abstention to answering from priors (contamination). The composite verdict flips
fully to \textsc{leaking} on the lighter model (\texttt{gemini-flash-lite-latest}); on the
stronger model (\texttt{gemini-flash-latest}) it lands \textsc{inconclusive}, because that
model continued to follow the counterfactual ``alter-to-port'' rule even without the binding
clause---a more capable instruction-follower whose composite verdict the counterfactual
sub-test holds back even as the ablation and closed-book signals both indicate leakage.
This is the discrimination the instrument is designed to expose, and the mock learners
(rows~1--2) bound the scale: known-corpus-bound $\to \delta{=}1.000$/abstain, known-leaking
$\to \delta{=}0.000$/contaminate.
...
\texttt{gemini-flash-latest} & unconstrained & $0.000$ & follows & answers & \textsc{inconclusive} \\
\texttt{gemini-flash-lite-latest} & bound & $0.667$ & follows & abstains & \textsc{corpus-bound} \\
\texttt{gemini-flash-lite-latest} & unconstrained & $0.000$ & ignores & answers & \textsc{leaking} \\
\bottomrule
\end{tabular}
\caption{C1 discrimination on two real LLMs (head-on instrument, $\delta$ threshold
$0.15$). Removing the corpus-binding clause collapses the ablation-$\delta$ to $0.000$ and
flips closed-book abstention to contamination for both models; the composite verdict flips
fully to \textsc{leaking} on the lighter model. Mock learners (top) bound the scale.}
\label{tab:disc}
\end{table}
...
analysis showing the protocol \emph{ranking} is stable is outstanding. \textbf{Model coverage.}
The discrimination result (Table~\ref{tab:disc}) covers two models; a broader sweep
(more models, and especially weak instruction-followers, which are the interesting leak
cases) would tighten the ``verdict flips'' claim. \textbf{Compute-gated:} the open-weight
