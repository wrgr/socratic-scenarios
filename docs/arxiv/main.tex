...
\paragraph{Real-model discrimination.} We run the necessity test on current production LLMs over
AWS Bedrock, contrasting a \emph{standard-COLREG} corpus (rules every model plausibly knows) with
the \emph{corpus-only charted hazard} of Section~\ref{sec:construction} (a fact no model can know
from pretraining), Table~\ref{tab:disc}. The verdict is a majority vote of three orthogonal
signals: low necessity, counterfactual ignored, closed-book contaminated. Two findings. \emph{(1)
Standard rules are redundant across the board.} Every frontier model reads textbook COLREG
\textsc{leaking/redundant}---the corpus duplicates parametric knowledge, so it adds no value. This
is itself an actionable audit verdict (do not ship these in the corpus), not an instrument failure,
and it is exactly why the weight-level dose-response uses a corpus-only hazard, where models have
dynamic range. \emph{(2) The hazard discriminates---along two axes.} Claude Haiku and Sonnet read
it \textsc{corpus-bound} (they use the corpus to avoid a danger they cannot otherwise see);
Llama-3.3-70B grounds regardless---its with-corpus regret stays at the full barrier
($\regret_{\text{with}}\!\approx\!1207$), so the item is not redundant but \textsc{unusable}
(present yet unexploited, the fix is the model not the corpus); Amazon Nova-Pro is erratic
(\textsc{inconclusive}). The \emph{redundant}-vs-\emph{unusable} split of point~(iii) is thus
observed on real models, not only on mocks. (A separate prompt-condition study on Gemini models
gives the binding-fidelity control: under a strict corpus-binding prompt necessity grades
monotonically with model capability, and removing the binding clause flips every model to
\textsc{leaking}---confirming the strict prompt is a genuine positive control.)
...
\begin{tabular}{@{}llcl@{}}
\toprule
Model & Standard COLREG & Charted hazard (corpus-only) & Hazard mechanism \\
\midrule
mock (corpus-bound) & --- & \textsc{corpus-bound} & reads the corpus \\
mock (leaking) & \textsc{redundant} & \textsc{leaking} & ignores the corpus \\
\midrule
Claude Haiku 4.5 & \textsc{redundant} & \textsc{corpus-bound} & reads \& avoids \\
Claude Sonnet & \textsc{redundant} & \textsc{corpus-bound} & reads \& avoids \\
Llama-3.3-70B & \textsc{redundant} & \textsc{leaking/unusable} & grounds regardless ($\regret_{\text{with}}\!\approx\!1207$) \\
Amazon Nova-Pro & mixed & \textsc{inconclusive} & erratic \\
\bottomrule
\end{tabular}
\caption{Cross-model necessity on AWS Bedrock: a standard-COLREG corpus vs.\ a corpus-only charted
hazard. Every frontier model reads the standard rules \textsc{redundant} (the corpus duplicates
parametric knowledge---an actionable audit verdict). The hazard, a fact no model can know from
pretraining, \emph{discriminates}: Claude models are \textsc{corpus-bound} (they use it),
Llama-3.3-70B is \textsc{unusable} (grounds even with the rule present---with-corpus regret at the
full barrier, so the item has value but the model cannot act on it), and Nova is
\textsc{inconclusive}. The \emph{redundant} vs.\ \emph{unusable} split is thus observed on real
models. Mock learners bound the scale. (Standard-corpus verdicts summarize the per-rule audit;
Nova reads one of three standard rules relied-on, hence \emph{mixed}.)}
\label{tab:disc}
\end{table}
