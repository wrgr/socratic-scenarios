...
audit-level half: with the NPO baseline (the CPU-tractable arm) on Qwen2.5-1.5B-Instruct---a
model that knows the rule---unlearning drives the forget-target NLL from $4.6$ to $41$ and
the held-out direction-cue rate (robust to the starboard$\to$right synonym) from $5/6$ to
$0/6$, while retain knowledge stays coherent. This establishes behavioral \emph{suppression},
not yet verified semantic \emph{removal}: the probe is lexical and the NLL teacher-forced.
Scoring the base-vs.-unlearned model on the full regret instrument at 7--8B scale with the
primary SimNPO arm---the step that turns suppression into an instrument-level removal result
and fills the corpus-recovery cell of the $2\times2$---remains GPU-gated.
