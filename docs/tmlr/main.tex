...
checkpoints---so their agreement rules out a gradient-method artifact. We establish the calibration in
a simulator-free QA domain, then use a decision-based domain to surface a limit the calibration alone
would hide.
...
\textbf{Single models and seeds.} The calibration dose-response and the knowing\,$\neq$\,doing
decision-domain results are single-model, single-seed runs; a queued multi-seed GPU rerun ($\geq\!3$
seeds, and a second model family) is documented in \texttt{experiments/provenance/gpu-pending.md} and
\texttt{decision-teach-arc.md}, and until it lands we treat the curves as single-run and keep the
hedging in Section~\ref{sec:necessity}. The offline mock recovery already confirms the \emph{instrument}
recovers ground truth on these domains; what is pending is the multi-seed \emph{taught-model} band. \textbf{Small evidence base.} The cross-model
...
a graded dose-response ($1.00\!\to\!0.03$, in a simulator-free QA domain, with LoRA-scale and
checkpoint gradients agreeing). A decision-based domain adds a validity caution the field should carry:
