...
smoothly $1.00\!\to\!0.93\!\to\!0.29\!\to\!0.03$ with the LoRA-scale and checkpoint gradients agreeing.
...
rules out a gradient-method artifact. This is the known-groups calibration, in a domain we did not
...
move}: $\regret$-necessity is flat ($\approx\!667$ at both the naive and taught ends). The fact is in
...
\bottomrule
\end{tabular}
\caption{Necessity dose-response by construction (the calibration). Teaching many independent invented
facts into the weights drives the instrument's necessity for them smoothly down the LoRA-$\alpha$
gradient; a training-checkpoint sweep reproduces the same curve, so the monotone fall is not a
gradient-method artifact. The answer-accuracy objective uses no simulator. The decision-based hazard
