...
| 1b | Cross-model **hazard** discrimination (class×size, 10 models) | C1(ii)/C1(iii) | ✅ result in hand | necessity spans **0→1998**; only **Opus** corpus-bound at full barrier; **Llama-70B/Nova-micro/gpt-oss-120B unusable** (ground with rule); standard COLREG **redundant for frontier but relied-upon by gpt-oss** (2–3/4) — necessity is a (corpus,learner) property. gpt-oss-20B hazard n/a (reasoning didn't terminate in budget) |
