...
| B2 | **Fact-vs-reasoning (generalization) probe** | C1 depth | ✅ built + tested | reasoning-aware objective (a corpus local rule redefines the governing give-way side, Rules 2/9) + a matched/conflict suite. Offline: implementer & reasoner **identical on matched** (Δregret 0.00); on conflict the **reasoner clears 6/6**, the lookup **implementer clears only the 3 reaches where the rule is redundant with Rule 14** and grounds on the 3 it overrides — necessity of the local-rule corpus = 3/6, separable *only* on the overriding cases. 6 tests. Live-model run pending |
